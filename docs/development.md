...
| `test_search.py` | `kairos/search.py` | `prepare_groups` (fixed/locked/accept resolution, bad-key errors, unknown-number error, empty-list-means-all, locked beats accept precedence, migration, slot-twin locking), `search`/`enumerate_clashfree`/`rank_arrangements` (footprint dedup, clash detection, config independence, twin collapsing/entangling), and the `score_raw`/`weight_scored`/arrangement-structure helpers behind the raw-cache split. |
