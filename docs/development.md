...
(16 files under `tests/`, one per row above — `__init__.py` and `conftest.py`
are support files, not test modules. The plan that seeded this doc said 14;
`ls tests/` at writing time shows 16, so the table above reflects the
current repo.)
...
- **`plans/`** — numbered, single-change micro-plans with a status table
  (`plans/README.md`); generated by an audit pass and executed one at a time.
